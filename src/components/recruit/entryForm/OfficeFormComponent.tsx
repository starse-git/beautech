...
import { sendOfficeMail } from "@/utils/actions/send-mail";
...
const OfficeFormComponent = () => {
  ...
  const formSchema = z.object({
    ...
    occupation: z.array(z.string()).min(1, "職種を選択してください!"),
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setLoading(true);
    try {
      const { success, error } = await sendOfficeMail(values);
      if (success) {
        form.reset();
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      className="border-gray-300"
                      placeholder={
                        "例：〒103-0004　東京都中央区東日本橋1丁目3-3　TYDビル 3F"
                      }
                      {...field}
                    />
                  </FormControl>
